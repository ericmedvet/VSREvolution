  ...
  public Function<TimedRealFunction, Robot> buildFor(Robot robot) {
    ...
    return function -> {
      if (function.getInputDimension() != 2) {
        throw new IllegalArgumentException(String.format("Wrong number of function input args: 2 expected, %d found",
            function.getInputDimension()
        ));
      }
      return new Robot(new PhaseSin(frequency, amplitude, Grid.create(body.getW(),
          body.getH(),
          (x, y) -> function.apply(
              0d,
