...
public class NamedFunctions {
  ...
  public static List<NamedFunction<Evolver.Individual<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?>> basicIndividualFunctions(
      Function<DevoOutcome, Double> fitnessFunction
  ) {
    NamedFunction<Evolver.Individual<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?> size = size().of(
        genotype());
    NamedFunction<Evolver.Individual<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ? extends Grid<?>> firstShape = f(
        "shape",
        (Function<Robot, Grid<?>>) Robot::getVoxels
    ).of(f("first", (Function<DevoOutcome, Robot>) l -> l.getRobots().get(0))).of(fitness());
    NamedFunction<Evolver.Individual<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ? extends Grid<?>> lastShape = f(
        "shape",
        (Function<Robot, Grid<?>>) Robot::getVoxels
    ).of(f("last", (Function<DevoOutcome, Robot>) l -> l.getRobots().get(l.getRobots().size() - 1))).of(fitness());
  ...
  public static List<NamedFunction<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?>> keysFunctions() {
    return List.of(eventAttribute("experiment.name"),
  ...
  public static List<NamedFunction<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?>> basicFunctions() {
    return List.of(iterations(), births(), fitnessEvaluations(), elapsedSeconds());
  }

  public static List<NamedFunction<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?>> populationFunctions(
      Function<DevoOutcome, Double> fitnessFunction
  ) {
    NamedFunction<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?> min = min(Double::compare).of(
        each(f("fitness", fitnessFunction).of(fitness()))).of(all());
    NamedFunction<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?> median = median(Double::compare).of(
        each(f("fitness", fitnessFunction).of(fitness()))).of(all());
  ...
  public static Accumulator.Factory<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, String> lastEventToString(
      Function<DevoOutcome, Double> fitnessFunction
  ) {
    ...
    final List<NamedFunction<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?>> functions = Misc.concat(
        List.of(keysFunctions(),
            basicFunctions(),
    ...
    return Accumulator.Factory.<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>>last()
        .then(e -> functions.stream()
            .map(f -> f.getName() + ": " + f.applyAndFormat(e))
            .collect(Collectors.joining("\n")));
  }

  public static Accumulator.Factory<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, BufferedImage> fitnessPlot(
      Function<DevoOutcome, Double> fitnessFunction
  ) {
    return new TableBuilder<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, Number>(List.of(
        iterations(),
  ...
  public static Accumulator.Factory<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, File> bestVideo(
    ...
    return Accumulator.Factory.<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>>last()
        .then(event -> {
          ...
          UnaryOperator<Robot> solution = Misc.first(event.orderedPopulation().firsts()).solution();
  ...
  public static List<NamedFunction<Evolver.Individual<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, ?>> visualIndividualFunctions() {
    return List.of();
  }

  public static Function<Evolver.Event<?, ? extends UnaryOperator<Robot>, ? extends DevoOutcome>, Collection<DevoValidationOutcome>> validation(
    ...
    return event -> {
      ...
      for (String validationTerrainName : validationTerrainNames) {
        for (int seed : seeds) {
          ...
          devoValidationOutcomes.add(new DevoValidationOutcome(event,
              Map.ofEntries(Map.entry("validation.terrain", validationTerrainName), Map.entry("validation.seed", seed)),
              outcomes
          ));
        }
      }
