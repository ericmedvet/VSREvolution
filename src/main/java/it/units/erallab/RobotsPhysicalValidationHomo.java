...
public class RobotsPhysicalValidationHomo extends Worker {

  public static void main(String[] args) {
    new RobotsPhysicalValidationHomo(args);
  }

  public RobotsPhysicalValidationHomo(String[] args) {
    super(args);
  }
  ...
  public void run() {
    ...
    List<Map<String, Double>> activeDeltaParams = sampleRange(activeDelta.second().first(), 8).stream().map(s ->
        Map.of("activeDelta", s)
    ).toList();
    Pair<String, Pair<DoubleRange, Integer>> springD = Pair.of("springD", Pair.of(DoubleRange.of(0.1, 0.99), 8));
    List<Map<String, Double>> springDParams = sampleRange(springD.second().first(), 8).stream().map(s ->
        Map.of("springD", s)
    ).toList();
    Pair<String, Pair<DoubleRange, Integer>> springF = Pair.of("springF", Pair.of(DoubleRange.of(3, 10), 8));
    List<Map<String, Double>> springFParams = sampleRange(springF.second().first(), 8).stream().map(s ->
        Map.of("springF", s)
    ).toList();
    Pair<String, Pair<DoubleRange, Integer>> friction = Pair.of("friction", Pair.of(DoubleRange.of(0.1, 0.3), 8));
    List<Map<String, Double>> frictionParams = FRICTION_VALUES.stream().map(s ->
        Map.of("friction", s)
    ).toList();

    //List<Map<String, Double>> validationProperties = buildVoxelPropertiesCombinations(List.of(activeDelta, springD, springF, friction));
    ...
    for (CSVRecord record : records) {
      ...
      if (expName.contains("active")) {
        newRobots.addAll(
            activeDeltaParams.stream().map(params -> new Robot(
        ...
        newRobots.addAll(
            springFParams.stream().map(params -> new Robot(
                SerializationUtils.clone(robot.getController()),
                changeRobotsPhysicalProperties(robot.getVoxels(), params)
            )).toList()
        );
        ...
        newRobots.addAll(
            springDParams.stream().map(params -> new Robot(
                SerializationUtils.clone(robot.getController()),
                changeRobotsPhysicalProperties(robot.getVoxels(), params)
            )).toList()
        );
      } else if (expName.contains("friction")) {
        newRobots.addAll(
            frictionParams.stream().map(params -> new Robot(
                SerializationUtils.clone(robot.getController()),
                changeRobotsPhysicalProperties(robot.getVoxels(), params)
            )).toList()
        );
  ...
  private List<Map<String, Double>> buildVoxelPropertiesCombinations(
    ...
    IntStream.range(0, 4).forEach(i -> {
          System.out.print(propertyNames.get(i) + " ");
